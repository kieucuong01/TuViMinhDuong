...
describe("AI reading format", () => {
  ...
  it("renders a useful instant free overview before the background LLM finishes", () => {
    ...
    expect(content).toContain("## 1. Khí chất cốt lõi");
    expect(content).toContain("## 2. Công việc và hướng đi");
    expect(content).toContain("## 3. Tài chính và nguồn lực");
    expect(content).toContain("## 4. Gia đình, quan hệ và nhịp sống");
    ...
    expect(content).toContain("Những gì bạn vừa đọc chỉ là bề nổi");
