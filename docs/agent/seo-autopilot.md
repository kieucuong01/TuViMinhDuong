...
15a. Only after the published URL returns `200` and appears in the live sitemap, write its slug and verification time to `docs/seo-autopilot/state.json` as `lastPublishedSlug` and `lastPublishedAt`. Planning fields such as `lastAction` must never be treated as proof of publication.
...
- Daily publisher runs use a fast live snapshot by default: sitemap/robots stay mandatory, but only the priority pages are sampled unless `--sample-size` is explicitly raised for a deeper audit.
...
- Merge the repo article inventory with live sitemap `/kien-thuc-tu-vi/*` slugs before topic selection. The merged set is the duplicate guard for both exact slug conflicts and overlapping reader-intent groups, so Codex does not recreate articles published by the VPS-side AI agent.
...
1. Daily/weekly audit: run `npm run seo:autopilot` or `npm run seo:autopilot:execute`, inspect live snapshot, sitemap, article inventory, SEMrush keyword clusters, and Search Console feedback when the warm-up phase is over. If the VPS-side AI agent has published an article that is live in sitemap but not yet in the local repo, live production wins for duplicate prevention.
